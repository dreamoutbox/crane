...
async fn test_user_state() {
    ...
    crane::commands::deploy::run(&user_present_config, user_present_config_path, true).await
    ...
    let user_absent_config = crane::config::read_config_toml_file(user_absent_config_path)
        .expect("Failed to load config");
    crane::commands::deploy::run(&user_absent_config, user_absent_config_path, true).await
        .expect("deploy failed");
