...
//     let patroni_config_data = build_patroni_config(node, pg_version, replica_pass, pg_nodes)?;
...
//         .unwrap_or_default();
//     let config_changed = existing_config.trim() != patroni_config_data.trim();

//     interactor.mkdir("/etc/patroni")?;
//     interactor.create_file("/etc/patroni/config.yml", &patroni_config_data)?;
